...
namespace DatabaseCodeGenrator
{
    public partial class Frm_MySql : Form
    {
        ...
        private void btn_Generate_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileLocation.ShowDialog() != DialogResult.Cancel)
            {
                ...
                progressBar1.Maximum += ((DataTable)DB.ExecuteSqlStatmentQuery(@"select * from information_schema.views where TABLE_SCHEMA = '" + DatabaseName + "'", DB_OperationProcessForMySql.ResultReturnedDataType.Table)).Rows.Count;

                progressBar1.Maximum += ((DataTable)DB.ExecuteSqlStatmentQuery(@"select * from information_schema.routines where routine_type = 'PROCEDURE' and ROUTINE_SCHEMA = '" + DatabaseName + "'", DB_OperationProcessForMySql.ResultReturnedDataType.Table)).Rows.Count;
                ...
                myTableOfTables = (DataTable)DB.ExecuteSqlStatmentQuery(@"select * from information_schema.routines where ROUTINE_TYPE = 'PROCEDURE' and ROUTINE_SCHEMA = '" + DatabaseName + "'", DB_OperationProcessForMySql.ResultReturnedDataType.Table);
                ...
                {
                    CurrentTable = TablesList.GetItemText(TablesList.Items[i]);
                    DataTable myTableOfPrams = (DataTable)DB.ExecuteSqlStatmentQuery(@"select PARAMETER_NAME, DATA_TYPE from information_schema.parameters where specific_name = '" + CurrentTable + "' and SPECIFIC_SCHEMA = '" + DatabaseName + "'", DB_OperationProcessForMySql.ResultReturnedDataType.Table);
                    ListBox ColumnsList = new ListBox();
                    Form frm2 = new Form();
                    frm2.Controls.Add(ColumnsList);
                    ...
                    for (int x = 0; x < ColumnsList.Items.Count; x++)
                    {
                        ...
                        string dataTypename = "";
                        dataTypename = DB.ExecuteSqlStatmentQuery(@"select DATA_TYPE from information_schema.parameters where SPECIFIC_NAME = '" + CurrentTable + "' and PARAMETER_NAME = '" + CurrentPram + "' and SPECIFIC_SCHEMA = '" + DatabaseName + "'", DB_OperationProcessForMySql.ResultReturnedDataType.Scalar).ToString();
                        dataTypename = GiveName(dataTypename);
                        ListOfPrams.Add(CurrentPram, dataTypename);
                    }
                    ListCalsses[ListCalsses.Count - 1].AddClassMySQLStordedProcedure(CurrentTable, "DataSet", ListOfPrams);
                    progressBar1.Value += 1;
                }
                ListCalsses[ListCalsses.Count - 1].CreateClassCode();
                #endregion
    ...
    public class DB_OperationProcessForMySql
    {
        ...
        public DB_OperationProcessForMySql()
        {
        }
        ...
        {
            _SqlParameters.Add(new MySqlParameter(Name, value));
        }
